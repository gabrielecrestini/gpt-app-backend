# main.py - Versione Finale, Corretta e Robusta
...
def sync_user(user_data: UserSyncRequest):
    try:
        ...
        if not response: raise Exception("La risposta dal database era nulla (None).")
...
def claim_streak_bonus(user_id: str):
    try:
        ...
        user_res = supabase.table('users').select('points_balance').eq('user_id', user_id).maybe_single().execute()
        if not user_res or not user_res.data: raise HTTPException(status_code=404, detail="Utente non trovato per accredito bonus.")
...
# ... (altri endpoint qui, li lascio invariati per non creare confusione ma la logica sopra è la più importante)
@app.post("/request_payout")
def request_payout(payout_data: PayoutRequest): raise HTTPException(status_code=501, detail="Non implementato.")
@app.get("/contests/current")
def get_current_contest(): return {"id": 1, "theme_prompt": "Un robot che dipinge un tramonto, stile Van Gogh"}
...
def generate_ai_image(req: ImageGenerationRequest):
    ...
    except Exception as e: print(f"Errore imprevisto in generate_image: {e}"); raise HTTPException(status_code=500, detail="Errore interno.")
...
def vote_for_submission(submission_id: int):
    supabase.rpc('increment_votes', {'submission_id_in': submission_id}).execute(); return {"status": "success"}
@app.get("/leaderboard")
def get_leaderboard():
...
def update_profile(user_id: str): raise HTTPException(status_code=501, detail="Non implementato.")