# main.py - Versione Finale, Corretta e Robusta con Logica di Riprova
# Data: 30 Giugno 2025
...
if all([GCP_PROJECT_ID, GCP_REGION, GCP_SA_KEY_JSON_STR]):
    ...
    print("ATTENZIONE: Credenziali Google Cloud non trovate.")
...
def sync_user(user_data: UserSyncRequest):
    ...
    for attempt in range(max_retries):
        try:
            print(f"Tentativo {attempt + 1} di sincronizzare l'utente {user_data.user_id}...")
            ...
            if response:
                ...
                print("Sincronizzazione utente riuscita.")
                return {"status": "success"}
            
            print(f"Tentativo {attempt + 1} fallito: risposta nulla dal database.")
        except Exception as e:
            print(f"Errore eccezionale nel tentativo {attempt + 1}: {e}")
        
        if attempt < max_retries - 1:
            print("Riprovo tra 1 secondo...")
            time.sleep(1)
...
def claim_streak_bonus(user_id: str):
    try:
        ...
        if not user_res or not user_res.data:
            raise HTTPException(status_code=404, detail="Utente non trovato per accreditare il bonus.")
